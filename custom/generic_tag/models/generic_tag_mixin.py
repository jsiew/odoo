...
class GenericTagMixin(models.AbstractModel):
    ...
    def _search_no_tag_id(self, operator, value):
        with_tags = self.search([('generic_tag_ids', operator, value)])
        return [('id', 'not in', with_tags.mapped('id'))]

    def _search_tag_id(self, operator, value):
        return [('generic_tag_ids', operator, value)]
    ...
    generic_tag_ids = fields.Many2many(
    ...
    def add_tag(self, code=None, name=None, create=False):
        ...
        if tags:
            self.write({'generic_tag_ids': [(4, t.id) for t in tags]})
    ...
    def remove_tag(self, code=None, name=None):
        ...
        if tags:
            self.write({'generic_tag_ids': [(3, t.id) for t in tags]})
    ...
    def check_tag(self, code=None, name=None):
        ...
        if code is not None:
            tag_domain.append(('generic_tag_ids.code', '=', code))
        if name is not None:
            tag_domain.append(('generic_tag_ids.name', '=', name))
    ...
    def check_tag_category(self, code=None, name=None):
        ...
        if code is not None:
            categ_domain.append(('generic_tag_ids.category_id.code', '=', code))
        if name is not None:
            categ_domain.append(('generic_tag_ids.category_id.name', '=', name))
